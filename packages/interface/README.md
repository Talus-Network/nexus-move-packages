...
Public types and function signatures shared by Nexus agents, DAGs, graphs, payments, authorization, schemas, results, and verifiers.
...
Add `nexus_interface` when application code directly uses the common Nexus domain model. It exposes these interface modules:
...
- `sui move build` reads these declarations and links the consumer to the selected network's published `nexus_interface` address.
...
Use a `#[test_only]` module extension under the TAP package's `tests/` directory when a test needs a private value shape. Module extensions require `edition = "2024.alpha"`.

An extension shares the scope of the module it extends, so it can add the smallest constructor, observation, or cleanup helper needed by the test. It cannot override an existing Nexus function. Calls to existing functions made by `nexus tap test` execute the selected published bytecode.
