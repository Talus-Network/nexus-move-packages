...
//! Interface for [`nexus_registry::agent_registry`].
//!
...
public native fun share_registry(registry: AgentRegistry);
...
public native fun create_agent(
...
public native fun attach_embedded_agent(
    registry: &mut AgentRegistry,
...
public native fun register_skill(
    registry: &mut AgentRegistry,
    agent: &mut nexus_interface::agent::Agent,
...
/// revision 1.
public native fun bootstrap_default_runtime_dag_skill_for_deployment(
    registry: &mut AgentRegistry,
    ctx: &mut sui::tx_context::TxContext,
): (sui::object::ID, u64);

/// Return requirements for the currently active interface revision.
public native fun get_skill_requirements(
...
/// recorded ID.
public native fun new_default_task_components(
    registry: &AgentRegistry,
...
public native fun default_dag_executor_agent_id_from_registry(
    registry: &AgentRegistry,
): sui::object::ID;
...
///
/// Aborts with [EDefaultDagExecutorMissing] if no default executor has been bootstrapped.
public native fun default_dag_executor_skill_id_from_registry(registry: &AgentRegistry): u64;

/// Returns whether the given `(agent_id, skill_id)` is the registry owned default DAG executor.
///
/// Aborts with [EDefaultDagExecutorMissing] if no default executor has been bootstrapped.
public native fun is_default_dag_executor_skill(
    registry: &AgentRegistry,
    agent_id: sui::object::ID,
    skill_id: u64,
): bool;
...
public native fun skill_pinned_dag_id_for_skill(
    registry: &AgentRegistry,
    agent_id: sui::object::ID,
    skill_id: u64,
...
public native fun skill_requirements_for_skill(
    registry: &AgentRegistry,
    agent_id: sui::object::ID,
    skill_id: u64,
): nexus_interface::agent::SkillRequirement;
...
/// Aborts with [ESkillNotFound] if the skill does not exist, or [EActiveEndpointNotFound]
/// if the agent or skill is inactive.
public native fun get_agent_skill_record(
    registry: &AgentRegistry,
    agent_id: sui::object::ID,
    skill_id: u64,
): (
...
public native fun classify_skill_contract(
    registry: &AgentRegistry,
    agent_id: sui::object::ID,
    skill_id: u64,
...
public native fun skill_contract_is_current(state: SkillContractState): bool;

/// Returns whether a skill contract may become admissible without changing the proposal.
public native fun skill_contract_is_inactive(state: SkillContractState): bool;
...
///
/// Aborts with [ESkillNotFound] if the agent has no such skill.
public native fun skill_dag_binding(
    registry: &AgentRegistry,
    agent: &nexus_interface::agent::Agent,
    skill_id: u64,
...
///
/// Aborts with [EAgentNotFound] if the agent is not registered.
public native fun set_skill_active(
    registry: &mut AgentRegistry,
    agent: &mut nexus_interface::agent::Agent,
    skill_id: u64,
    active: bool,
...
///
/// Aborts with [EAgentNotFound] if the agent is not registered.
public native fun set_agent_active(
    registry: &mut AgentRegistry,
    agent: &mut nexus_interface::agent::Agent,
    active: bool,
);
...
///
/// Aborts with [EAgentNotFound] if the agent is not registered.
public native fun update_skill_description(
    registry: &mut AgentRegistry,
    agent: &mut nexus_interface::agent::Agent,
    skill_id: u64,
    description: vector<u8>,
...
///
/// Aborts with [EAgentNotFound] if the agent is not registered.
public native fun update_skill_policies(
    registry: &mut AgentRegistry,
    agent: &mut nexus_interface::agent::Agent,
    skill_id: u64,
    payment_policy: nexus_interface::payment::SkillPaymentPolicy,
